...
def consolidate_null_unidentified(apps, schema_editor):
  ...
  qs_un_doctypes = (DocumentType
                    .objects
                    .filter(title="Unidentified"))
  (DataDocument
   .objects
   .filter(document_type__in=qs_un_doctypes)
...
def fwd_load_compatibility(apps, schema_editor):
    ...
    '''
    DocumentType = apps.get_model('dashboard', 'DocumentType')
    DocumentTypeGroupTypeCompatibilty = apps.get_model('dashboard', 'DocumentTypeGroupTypeCompatibilty')
...
def update_datadoc_doctype(apps, schema_editor):
    ...
    DocumentType = apps.get_model('dashboard', 'DocumentType')
    DocumentTypeGroupTypeCompatibilty = apps.get_model('dashboard', 'DocumentTypeGroupTypeCompatibilty')
    qs_doctype_old = (DocumentType
                      .objects
                      .exclude(id__in=(DocumentTypeGroupTypeCompatibilty
                                       .objects
                                       .values('document_type_id'))))
    qs_doctype_new = (DocumentType
                      .objects
                      .filter(id__in=(DocumentTypeGroupTypeCompatibilty
                                      .objects
                                      .values('document_type_id'))))
    for doctype_old in qs_doctype_old:
        doctype_new = (qs_doctype_new
                       .filter(title=doctype_old.title)
                       .get())
        (DataDocument
         .objects
         .filter(document_type=doctype_old)
         .update(document_type=doctype_new))
...
def rm_nonunique_doctypes(apps, schema_editor):
    ...
    (DocumentType
     .objects
     .exclude(
         id__in=(DocumentTypeGroupTypeCompatibilty
                 .objects
                 .values('document_type_id'))
     )
     .delete())
...
class Migration(migrations.Migration):
    ...
    operations = [
        ...
        migrations.AddField(
            model_name='documenttype',
            name='group_types',
            field=models.ManyToManyField(related_name='groups', through='dashboard.DocumentTypeGroupTypeCompatibilty', to='dashboard.GroupType'),
        ),
    ]

...
class TestProductPuc(TestCase):
    ...
    def test_admin_puc_tag_column_exists(self):
        ...
        response = self.client.get(
            reverse('admin:dashboard_puctotag_changelist'))
        response_html = html.fromstring(response.content.decode('utf8'))
        # import pdb; pdb.set_trace()
        tag_col = response_html.xpath(
            '//th[contains(@class, "column-tag")]/div/a')[0].text
        self.assertEqual(
            'Tag', tag_col, "Tag should be a column in the table!")
        tag_filter = response_html.xpath(
            '//div[contains(@id, "changelist-filter")]/h3')[0].text
        self.assertEqual(tag_filter, ' By tag ', "Filter by tag!")

    def test_admin_puc_change(self):
        puc = PUC.objects.get(pk=1)
        puc_response_url = reverse(
            'admin:dashboard_puc_change', args=(puc.pk,))
        ...
        product_response_html = html.fromstring(
        ...
        assumed = product_response_html.xpath(
    ...
    def test_bulk_product_puc_ui(self):
        ...
        product_response = self.client.get(
            product_response_url + '?q=ewedwefwefds')
        product_response_html = html.fromstring(
            product_response.content.decode('utf8'))
        self.assertIn('Locate products to associate with PUCs using the Search bar above.',
                      product_response_html.xpath('string(/)'),
    ...
    def test_bulk_product_puc_post(self):
        ...
        puc = PUC.objects.get(pk=1)
        self.assertEqual(product.get_uber_puc(), puc,
        ...
        self.assertTrue(
            p2p, "Product 11 should also be assigned to PUC 1 with a classification method of MB")
    ...
    def test_product_puc_duplicate(self):
        ...
        puc = PUC.objects.get(pk=1)
        p2p = ProductToPUC.objects.filter(
            product=product, puc=puc, classification_method='MA').first()
        updated_at = p2p.updated_at
        self.assertTrue(
            p2p, "Product 11 should also be assigned to PUC 1 with a classification method of MA")
    ...
    def test_bulk_product_tag_post(self):
        ...
        self.assertEqual(
            tag_count, 5, "Product 11 should now be assigned to 5 Tags")
    ...
    def test_bulk_product_tag_filter(self):
        ...
        response = self.client.post(product_response_url,
                                    {'puc': 1})
        response_html = html.fromstring(response.content.decode('utf8'))
        select = response_html.xpath(
            '//select[contains(@id, "id_tag")]/option')
        texts = [x.text for x in select]
        for attr in assumed_attrs:
            self.assertNotIn(
                attr, texts, "assumed attributes shouldn't be here")
        # import pdb; pdb.set_trace()

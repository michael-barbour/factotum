...
class QATest(TestCase):
    ...
    def test_qa_scoreboard(self):
        ...
        script_url = response_html.xpath(
            '//*[@id="extraction_script_table"]/tbody/tr['
            + str(row_count)
            + "]/td[1]/a/@href"
        )[0]
        self.assertEqual(
        ...
        displayed_doc_count = response_html.xpath(
            '//*[@id="extraction_script_table"]/tbody/tr[' + str(row_count) + "]/td[2]"
        )[0].text
        model_doc_count = DataDocument.objects.filter(
            extractedtext__extraction_script=self.objects.exscript.pk
        ).count()
        ...
        displayed_pct_checked = response_html.xpath(
            '//*[@id="extraction_script_table"]/tbody/tr[' + str(row_count) + "]/td[3]"
        )[0].text
        model_pct_checked = self.objects.exscript.get_pct_checked()
        self.assertEqual(
            displayed_pct_checked,
            model_pct_checked,
        ...
        self.assertEqual(
            es.get_qa_complete_extractedtext_count(),
            0,
            (
        ...
        script_qa_link = response_html.xpath(
            '//*[@id="extraction_script_table"]/tbody/tr[contains(.,"Test Extraction Script")]/td[5]/a/@href'
        )[0]
        self.assertIn(
        ...
        self.assertEqual(
            es.qa_begun, False, "The qa_begun property of the Script should be False"
        )
        ...
        self.assertIn(
            es.title,
            response_html.xpath("/html/body/div/h1/text()")[0],
        ...
        script_qa_status = response_html.xpath(
            '//*[@id="extraction_script_table"]/tbody/tr[contains(.,"Test Extraction Script")]/td[5]/text()'
        )[0]

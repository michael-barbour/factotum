...
def data_group_detail(request, pk,
    ...
    if request.method == 'POST' and 'clean_comp_data_button' in request.POST:
        ...
        if clean_comp_data_form.is_valid():
            ...
            for i, row in enumerate(csv.DictReader(info)):
                try:
                    extracted_chemical = ExtractedChemical.objects.get(rawchem_ptr_temp=int(row['id']))
                except ExtractedChemical.DoesNotExist as e:
                    extracted_chemical = None
                    context['clean_comp_err'][i + 1] = {'id': ['No ExtractedChemical matches rawchem_ptr_temp_id ' + row['id'], ]}
                    print('No ExtractedChemical matches rawchem_ptr_temp_id %s' % row['id'])
                try:
                    ingredient = Ingredient.objects.get(rawchem_ptr_temp=extracted_chemical.rawchem_ptr_temp)
                except Ingredient.DoesNotExist as e:
                    ingredient = Ingredient(rawchem_ptr_temp=extracted_chemical.rawchem_ptr_temp)

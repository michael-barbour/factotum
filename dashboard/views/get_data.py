...
def stats_by_dtxsids(dtxs):
    ...
    pucs_n = DSSToxSubstance.objects.filter(sid__in=dtxs).\
        values('sid').annotate(pucs_n=Count('rawchem_ptr_temp__extracted_chemical__extracted_text__data_document__product__puc')).values('sid','pucs_n')
    #print('pucs_n:')
    #print(pucs_n)
    ...
    dds_n = DSSToxSubstance.objects.filter(sid__in=dtxs).values('sid').\
        annotate(dds_n=Count('rawchem_ptr_temp__extracted_chemical__extracted_text__data_document')).values('sid','dds_n')
    #print('dds_n:')
    #print(dds_n)
    ...
    .annotate(
        dds_wf_n = Subquery(
            ExtractedChemical
            .objects
            .filter(rawchem_ptr_temp_id=OuterRef('rawchem_ptr_temp_id') )
            .filter(
                Q(raw_max_comp__isnull=False) |
                Q(raw_min_comp__isnull=False) |
                Q(raw_central_comp__isnull=False)
            )
            .values('rawchem_ptr_temp__extracted_chemical__extracted_text_id')
    ...
    strsql = ("SELECT dss.sid , "
                "IFNULL("
                    "SUM( "
                        ...
                        "WHERE ec2.rawchem_ptr_temp_id = dss.rawchem_ptr_temp_id "
                        "GROUP BY ec2.rawchem_ptr_temp_id "
                        "HAVING SUM( ( "
                            ...
                            "on ec.rawchem_ptr_temp_id = dss.rawchem_ptr_temp_id "
                            "GROUP BY dss.sid ")
    ...
    products_n = DSSToxSubstance.objects.filter(sid__in=dtxs).values('sid').\
       annotate(products_n=Count('rawchem_ptr_temp__extracted_chemical__extracted_text__data_document__product')).values('sid', 'products_n')
